...
_attrs = dict(source='source', target='target', name='id',
              key='key', link='links', label = 'label')


def node_link_data(G, attrs=None, dem_labels=None):
    ...
    name = attrs['name'] #这个name就是node ID
    source = attrs['source']
    target = attrs['target']
    links = attrs['link']
    lab = attrs['label']
    ...
    data = {'directed': G.is_directed(), 'multigraph': multigraph, 'graph': G.graph,
            # 'nodes': [dict(chain(Graph_nodes.items(), [(name, label),(lab,label)])) for label in Graph_nodes]}
            'nodes':   [dict(chain(G.nodes[n].items(), [(name, n), (lab, G.node[n]['label_test'])])) for n in G ]}
                     # dict(chain(dem_labels[i].items(), [(label, i)])) for i in G.nodes()]}
                     #   for n,i in dem_labels]}
    if multigraph:
        data[links] = [
            dict(chain(d.items(),
